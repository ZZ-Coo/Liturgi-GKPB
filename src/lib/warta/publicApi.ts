...
export async function fetchPublicWarta(slug: string, tanggal: string, code: string | null = null): Promise<WartaSection[]> {
  const { data, error } = await supabase.rpc('get_public_warta', { p_slug: slug, p_tanggal: tanggal, p_code: code })
